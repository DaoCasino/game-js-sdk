...
export class PlatformBackendConnection {
    ...
    private clientOnClose: (ev: CloseEvent) => any = undefined;

    public async listen(onEvent: () => any, onClose: (closeEvent: CloseEvent) => any): Promise<Api> {
        if (this.closed)
    ...
    private onClose(ev: CloseEvent) {
        this.closed = true;
        if (this.clientOnClose)
            this.clientOnClose(ev);
        this.api.close();
    }

    private constructor(wsUrl: string, httpUrl: string, authData: AuthData, webSocket: WebSocket) {
        this.wsUrl = wsUrl;
    ...
    static async init(wsUrl: string, httpUrl: string, authData: AuthData): Promise<PlatformBackendConnection> {
        const webSocket = new WebSocket(wsUrl);
        await new Promise((resolve, reject) => {
            webSocket.onopen = resolve;
            webSocket.onclose = reject
        });
        const conn = new PlatformBackendConnection(wsUrl, httpUrl, authData, webSocket)
        await conn.api.auth(authData.accessToken);
        return conn;
    }
}

export async function connect(url: string, account: string, secure = true): Promise<PlatformBackendConnection> {
